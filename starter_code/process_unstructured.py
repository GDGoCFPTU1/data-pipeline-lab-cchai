...
def process_pdf_data(raw_json: dict) -> dict:
    ...
    cleaned_content = re.sub(r'HEADER_PAGE_\d+\s*', '', raw_text)
    cleaned_content = re.sub(r'\s*FOOTER_PAGE_\d+', '', cleaned_content).strip()
    ...
    return {
        ...
        "source_type": "PDF",
        "author": str(raw_json.get("author", "Unknown")),
